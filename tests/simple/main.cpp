...
inline void GenericExample(Client& client) {
    ...
    client.Execute("CREATE TABLE IF NOT EXISTS test.t2 (id UInt64, name String) ENGINE = TinyLog");
    ...
    /// Insert some values.
    {
        ...
        // client.Insert("t2", block);
    }

    // Select values inserted in the previous step.
    client.Select("select toString(sum(id)), name from t2 group by name limit 1", [](const Block& block)
        {
            ...
            std::cout <<"other:" << rows << "\n"; 
            int cc = block.GetColumnCount();

            for (size_t c = 0; c < block.GetRowCount(); ++c) {
                for (int i=0; i<cc; i++)
                {
                    // std::cout << (*block[0]->As<ColumnUInt64>())[c] << " ";
                    //std::cout << (*block[i]->As<ColumnString>())[c] << " ";
                    std::string_view str = (*block[i]->As<ColumnString>())[c];
                    const char * p = std::string(str).c_str();
                    std::cout << p << ' ';
                    // std::cout << (*block[i]->As<ColumnString>())[c] << " ";
                }
    ...
    /// Delete table.
    //client.Execute("DROP TABLE test.t2");
}
