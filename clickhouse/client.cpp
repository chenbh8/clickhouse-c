...
extern "C" ck_res_t* ck_select(Client *client, char* query) {
    /// Create a table.
    static int error = 0;
    static ck_res_t* res = (ck_res_t*)ngx_alloc(sizeof(ck_res_t));
    memset((char*)res, 0, sizeof(res));
    if (res == NULL)
        return NULL;
    res->p = ngx_create_pool(1024);
    if (res->p == NULL)
        goto error;
        

    // Select values inserted in the previous step.
    // client->Select("select toString(sum(id)), name from t2 group by name limit 3", [](const Block& block)
    client->Select(query, [](const Block& block)
        
        {
            size_t cc = block.GetColumnCount();
            ck_rows_t * ck_rows = NULL;
            static ck_rows_t * ck_rows_pre = NULL;//为什么要用static，因为这个回调函数是多次调用，
            
            for (size_t c = 0; c < block.GetRowCount(); ++c)
            {
                res->nums++;
                if (!(ck_rows = (ck_rows_t*)ngx_palloc(res->p, sizeof(ck_rows_t))) || 
                !(ck_rows->data = (char**)ngx_palloc(res->p, sizeof(char**)*cc)))
                {
                    error = 1;
                    return;
                }
                ...
                {
                    std::string str = std::string((*block[i]->As<ColumnString>())[c]);
                    int len = str.length();
                    if (!(ck_rows->data[i] = (char*)ngx_palloc(res->p, len+1)))
                    {
                        error = 1;
                        return;
                    }
                    ...
                    // std::cout << str << ' ';
                    memcpy(ck_rows->data[i], str.c_str(), len);
                    *((ck_rows->data[i])+len) = '\0';
                }
...
extern "C" Client *ck_init()
{
    try { 
        const auto localHostEndpoint = ClientOptions()
                .SetHost(           getEnvOrDefault("CLICKHOUSE_HOST",     "192.168.0.213"))
                .SetPort(   getEnvOrDefault<size_t>("CLICKHOUSE_PORT",     "9000"))
                .SetUser(           getEnvOrDefault("CLICKHOUSE_USER",     "default"))
                .SetPassword(       getEnvOrDefault("CLICKHOUSE_PASSWORD", ""))
                .SetDefaultDatabase(getEnvOrDefault("CLICKHOUSE_DB",       "test"));


        return new Client(ClientOptions(localHostEndpoint)
